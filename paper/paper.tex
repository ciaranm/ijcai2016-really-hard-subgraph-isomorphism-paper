...
\begin{figure}[b]
    ...
    \begin{tikzpicture}[scale=0.45,every node/.style={font=\footnotesize}]
        \begin{scope}
            \node [draw, circle, fill=uofgcobalt, inner sep=1pt] (Na) at (1,  0) {1};
            \node [draw, circle, fill=uofgpumpkin, inner sep=1pt] (Nb) at (1, -1.5) {2};
            \node [draw, circle, fill=uofglawn, inner sep=1pt] (Nc) at (0, -3) {3};
            \node [draw, circle, fill=uofgthistle, inner sep=1pt] (Nd) at (2, -3) {4};
            ...
            \node [draw, circle, fill=uofgcobalt, inner sep=1pt] (N1) at (5.5,  0) {1};
            \node [draw, circle, fill=white, inner sep=1pt] (N2) at (7.5,  0) {2};
            \node [draw, circle, fill=uofgpumpkin, inner sep=1pt] (N3) at (5.5, -1.5) {3};
            \node [draw, circle, fill=white, inner sep=1pt] (N4) at (7.5, -1.5) {4};
            \node [draw, circle, fill=uofglawn, inner sep=1pt] (N5) at (5.5, -3) {5};
            \node [draw, circle, fill=uofgthistle, inner sep=1pt] (N6) at (7.5, -3) {6};
            ...
            \node [draw, circle, fill=white, inner sep=1pt] (M1) at (-5.5,  0) {1};
            \node [draw, circle, fill=uofgcobalt, inner sep=1pt] (M2) at (-3.5,  0) {2};
            \node [draw, circle, fill=uofgthistle, inner sep=1pt] (M3) at (-5.5, -1.5) {3};
            \node [draw, circle, fill=white, inner sep=1pt] (M4) at (-3.5, -1.5) {4};
            \node [draw, circle, fill=uofglawn, inner sep=1pt] (M5) at (-5.5, -3) {5};
            \node [draw, circle, fill=uofgpumpkin, inner sep=1pt] (M6) at (-3.5, -3) {6};
...
using GCC 4.9. Each uses backtracking search to build up an assignment of target vertices (values)
to pattern vertices (variables), with different kinds of inference and ordering heuristics.
...
We also implemented (but do not plot) an equivalent SAT encoding with direct encoding of the
cardinality constraints.  Using the Glucose solver, we saw very similar performance to the PB
encoding. We also implemented a similar integer program encoding; the Gurobi solver was only able to
solve some of the trivial satisfiable instances, and was almost never able to prove unsatisfiability
...
In future work, we intend to repeat the experiments using other random models, including bounded and
regular degree. We will also look at dynamic heuristics, and switching pattern and target heuristics
independently. Finally, we intend to investigate whether variance can be calculated efficiently
enough to give better predictions for very sparse or dense pattern graphs.
