...
instances are satisfiable. (We discuss the black line below.)
...
\textnormal{.} \] This formula predicts a very sharp phase transition from $\langle Sol \rangle \ll
1$ to $\langle Sol \rangle \gg 1$. We plot where this occurs using black lines in the first row of
...
?? Note that the size of the state space is defined differently to in binary CSPs.

\subsection{Variable and Value Ordering Heuristics}

?? Note that the Glasgow heuristics are moving us towards the satisfiable region, or increasing the
expected number of solutions \citep{Gent:1996:EN}.

?? LAD does SDF with no tiebreaking, no value ordering heuristic.

?? In contrast, VF2 heuristics work by selecting vertices adjacent to vertices already selected.

?? I suppose we should mention kappa \citep{Gent:1996:Kappa}, etc, here. This whole thing is a bit
subtle: maximising E(N) or Kappa as a principle is encoding-independent, but whether this is done by
SDF, deg, etc, is encoding-dependent.
...
?? This suggests that we're not able to make use of adjacency and non-adjacency simultaneously. Note
that this is solver-independent. In the following section we see whether this is an oddity of
...
?? Explain why we might guess degree heuristics just won't work for induced.

?? Explain bottom row of \cref{figure:induced}, and give way of calculating it.
...
solver, and the fourth row shows the number of search nodes used by a clique
encoding.}\label{figure:alt}
\end{figure*}

?? The odd stuff in the middle, which isn't near a phase transition, but is hard.
...
inside the central region, and that the easy satisfiable instances remain easy.
...
easy satisfiable instances become hard. Possible weakness in variable ordering heuristics.
